...
class AudioConsumer(AsyncWebsocketConsumer):
    ...
    async def receive(self, text_data=None, bytes_data=None):
        if text_data:
            ...
            # client is sending text data, this only happens initially
            self.messages = json.loads(text_data)['messages']
            self.bot_uuid = json.loads(text_data)['bot_uuid']
            print(f"01 - Received messages from {self.bot_uuid}: {self.messages}")
            # bot = sync_to_async(models.Bot.objects.get)(uuid=self.bot_uuid)
            # self.bot_model = bot.model
            self.bot_model = "gpt-4o-mini"
            print(f"01.5 - Found bot model {self.bot_model}")
