from django.db import models
import uuid


class Setting(models.Model):
    id = models.AutoField(primary_key=True)
    setting_key = models.CharField(max_length=50, unique=True)
    label = models.CharField(max_length=50)
    int_val = models.IntegerField(null=True)
    txt_val = models.CharField(max_length=250, null=True)
    is_txt = models.BooleanField(default=False)

    def __str__(self):
        return f"{self.setting_key}-{self.int_val}{self.txt_val}"

    class Meta:
        db_table = 'setting'


class Bot(models.Model):
    id = models.CharField(primary_key=True, max_length=36, default=uuid.uuid4)
    title = models.CharField(max_length=40, null=True) 
    ingress = models.TextField(null=True)  
    prompt = models.TextField(null=True)
    model = models.ForeignKey('BotModel', on_delete=models.RESTRICT, related_name="bots", null=True)
    temperature = models.DecimalField(max_digits=2, decimal_places=1, default=1.0)
    avatar_scheme = models.CharField(max_length=50, null=True)
    is_prompt_visible = models.BooleanField(default=True)
    is_library_bot = models.BooleanField(default=False)
    is_audio_enabled = models.BooleanField(default=False)
    owner = models.CharField(max_length=50, null=True)
    is_distribution_allowed = models.BooleanField(default=True)
    is_mandatory = models.BooleanField(default=False)
    is_img_bot = models.BooleanField(default=False)
    bot_info = models.TextField(null=True)

    def __str__(self):
        return f"{self.id}-{self.title}"
    
    class Meta:
        db_table = 'bot'


class BotModel(models.Model):
    id = models.AutoField(primary_key=True)
    deployment_key = models.CharField(max_length=36, null=True)
    provider = models.CharField(max_length=50)
    model_url = models.CharField(max_length=500, null=True)
    display_name = models.CharField(max_length=500, null=True)
    model_description = models.TextField(null=True)
    training_cutoff = models.CharField(max_length=200, null=True)
    retirement = models.CharField(max_length=200, null=True)
    filter = models.CharField(max_length=50, null=True)
    version = models.CharField(max_length=50, null=True)

    def __str__(self):
        return f"{self.deployment_key}-{self.display_name}"

    class Meta:
        db_table = 'bot_model'


class Favorite(models.Model):
    id = models.AutoField(primary_key=True)
    bot = models.ForeignKey(Bot, on_delete=models.CASCADE, related_name="favorites")
    feide_user = models.CharField(max_length=50)

    class Meta:
        db_table = 'favorite'
        unique_together = ('id', 'feide_user')


class PromptChoice(models.Model):
    id = models.CharField(max_length=7, primary_key=True)
    bot = models.ForeignKey(Bot, on_delete=models.CASCADE, related_name="prompt_choices")
    label = models.CharField(max_length=50)
    order = models.IntegerField()
    text = models.TextField(null=True) 

    class Meta:
        db_table = 'prompt_choice'

class ChoiceOption(models.Model):
    id = models.CharField(max_length=7, primary_key=True)
    choice = models.ForeignKey(PromptChoice, on_delete=models.CASCADE, related_name="options")
    label = models.CharField(max_length=50)
    text = models.TextField(default="")
    order = models.IntegerField()
    is_default = models.BooleanField(default=False)

    class Meta:
        db_table = 'choice_option'

class School(models.Model):
    class AccessEnum(models.TextChoices):
        NONE = 'none'
        EMP = 'emp'
        ALL = 'all'
        LEVELS = 'levels'

    id = models.AutoField(primary_key=True)
    org_nr = models.CharField(max_length=20, unique=True)
    name = models.CharField(max_length=50, null=True, default='')  
    code = models.CharField(max_length=3, null=True)  
    access = models.CharField(max_length=10, choices=AccessEnum.choices, default=AccessEnum.NONE)

    def __repr__(self):
        return f"{self.org_nr}-{self.name}"

    class Meta:
        db_table = 'school'
        ordering = ['name']


class BotAccess(models.Model):
    class AccessEnum(models.TextChoices):
        NONE = 'none'
        EMP = 'emp'
        ALL = 'all'
        LEVELS = 'levels'

    id = models.AutoField(primary_key=True)
    bot = models.ForeignKey(Bot, on_delete=models.CASCADE, related_name="accesses")
    school = models.ForeignKey(
        School, on_delete=models.CASCADE, related_name="accesses")
    level = models.CharField(max_length=20, null=True) 
    access = models.CharField(max_length=10, choices=AccessEnum.choices, default=AccessEnum.NONE, null=True) 

    def __str__(self):
        return f"{self.bot}-{self.school}{self.level}"

    class Meta:
        db_table = 'bot_access'
        unique_together = ('bot', 'school')


class BotLevel(models.Model):
    id = models.AutoField(primary_key=True)
    bot_access = models.ForeignKey(
        BotAccess, on_delete=models.CASCADE, db_column='access_id', related_name="levels")
    level = models.CharField(max_length=20, default='') 

    def __str__(self):
        return f"{self.id}-{self.bot_access}{self.level}"
    
    class Meta:
        db_table = 'bot_level'


class SchoolAccess(models.Model):
    id = models.AutoField(primary_key=True)
    school = models.ForeignKey(
        School, on_delete=models.CASCADE, related_name="school_accesses")
    level = models.CharField(max_length=20)

    def __str__(self):
        return f"global-{self.id}:{self.school}{self.level}"

    class Meta:
        db_table = 'school_access'


class SubjectAccess(models.Model):
    id = models.AutoField(primary_key=True)
    bot = models.ForeignKey(Bot, on_delete=models.CASCADE, related_name="subjects")
    subject_id = models.CharField(max_length=200, null=True) 
    created = models.DateTimeField(auto_now_add=True) 
    valid_from = models.DateTimeField(null=True)
    valid_to = models.DateTimeField(null=True)

    class Meta:
        db_table = 'subject_access'
        unique_together = ('bot', 'subject_id')


class PageText(models.Model):
    id = models.CharField(max_length=10, primary_key=True)
    title = models.CharField(max_length=50)
    text = models.TextField(null=True) 
    is_public = models.BooleanField() 

    class Meta:
        db_table = 'page_text'


class Role(models.Model):
    class RoleEnum(models.TextChoices):
        ADMIN = 'admin'
        AUTHOR = 'author'
        EMP = 'emp'

    id = models.AutoField(primary_key=True)
    feide_user = models.CharField(max_length=50, unique=True)
    user_name = models.CharField(max_length=50, null=True)
    role = models.CharField(max_length=10, choices=RoleEnum.choices, default=RoleEnum.EMP)
    school = models.ForeignKey(School, on_delete=models.CASCADE, related_name="roles", null=True) 

    def __str__(self):
        return f"{self.feide_user}-{self.role}"

    class Meta:
        db_table = 'role'


class UseLog(models.Model):
    class InteractionTypeEnum(models.TextChoices):
        TEXT = 'text'
        AUDIO = 'audio'

    id = models.AutoField(primary_key=True)
    role = models.CharField(max_length=20)
    level = models.IntegerField(null=True)
    bot_id = models.CharField(max_length=36)
    message_length = models.IntegerField(null=True)
    timestamp = models.DateTimeField(auto_now_add=True)
    interaction_type = models.CharField(max_length=10, choices=InteractionTypeEnum.choices, default=InteractionTypeEnum.TEXT, null=False)

    class Meta:
        db_table = 'use_log'
        ordering = ['-timestamp']


class LogSchool(models.Model):
    id = models.AutoField(primary_key=True)
    use_log = models.ForeignKey(UseLog, on_delete=models.CASCADE, related_name="schools")
    school_org_nr = models.CharField(max_length=20, null=True)

    class Meta:
        db_table = 'log_school'
        # unique_together = ('use_log', 'school_org_nr')


class TagCategory(models.Model):
    id = models.AutoField(primary_key=True)
    name = models.CharField(max_length=50)
    order = models.IntegerField(unique=True)

    def __str__(self):
        return f"{self.id}-{self.name}"

    class Meta:
        db_table = 'tag_category'


class TagLabel(models.Model):
    id = models.AutoField(primary_key=True)
    name = models.CharField(max_length=50)
    weight = models.IntegerField(null=True)
    order = models.IntegerField()
    category = models.ForeignKey(TagCategory, on_delete=models.CASCADE, related_name="tag_labels")

    def __str__(self):
        return f"{self.id}-{self.name}"
    
    class Meta:
        db_table = 'tag_label'
        unique_together = ('order', 'category')


class Tag(models.Model):
    id = models.AutoField(primary_key=True)
    value = models.IntegerField()
    category = models.ForeignKey(TagCategory, on_delete=models.CASCADE, related_name="tags")
    bot = models.ForeignKey(Bot, on_delete=models.CASCADE, related_name="tags")


    def __str__(self):
        return f"{self.id}-{self.category}{self.bot}"
    
    class Meta:
        db_table = 'tag'
        unique_together = ('category', 'bot')
