...
const sendMessage = async () => {
  try {
    ...
    image.value = '';
    spinner.value = true;
    const { data } = await axios.post('/api/send_img_message', { uuid: botId.value, prompt: prompt.value });
    image.value = data;
...
<template>
  ...
  <div class="d-flex justify-content-end">
    <RouterLink v-if="bot.distribute && bot.library" class="btn oslo-btn-secondary" :to="'/editbot/distribute/'+bot.uuid">
      Gi tilgang
    </RouterLink>
  </div>
  <h1 class="h2 mb-3">
    ...
    <ul class="list-group list-group-flush">
      ...
      <li class="container-fluid list-group-item response assistant">
        <span class="row">
          ...
          <div v-if="image.url" class="col-6">
          </div>
  ...
  <div id="input_line" class="mt-3">
    <div v-if="image != ''">
      Du kan redigere ledeteksten jeg brukte for å forbedre bildet:
    </div>
    <textarea id="text-input" ref="textInput" type="text" rows="5" aria-label="Forklar hva bildet skal vise. Ikke legg inn personlige og sensitive opplysninger." v-model="insertPrompt" class="form-control" placeholder="Forklar hva bildet skal vise. Ikke legg inn personlige og sensitive opplysninger." @keypress.enter.exact="sendMessage()"></textarea>
    <div class="card">
      <div class="card-body bg-body-tertiary">
        <button class="btn oslo-btn-primary" type="button" id="button-send" @click="sendMessage()">Send</button>
        <button class="btn oslo-btn-secondary" type="button" id="button-new" @click="resetMessages()">Start på nytt</button>
      </div>
    </div>
  </div>
